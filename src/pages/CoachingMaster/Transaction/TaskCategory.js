...
import url from "../../../utilities/files/Taskcategory.csv";
...
const TaskCategory = () => {
    ...
    const [stringData, setIsString] = useState("");
    const hiddenFileInput = React.useRef(null);

    useEffect(() => {
        const fetchData = () => {
            getData(`/TaskCategory/GetByDivision`, "TaskCategories-Index")
                .then((res) => {
                    if (res) {
                        setResults(res.data);
                    }
    ...
    const uploadFile = async () => {
        setWaiting(true);
        let data = await postData(`/TaskCategory/CreateList`, uploads, "TaskCategories-Index");
        if (data) {
    ...
    const handleDownload = (url) => {
        axios
            ...
            .then((res) => {
                fileDownload(res.data, "TaskCategory.csv");
            });
    };
    const openUpload = () => {
    ...
    const saveResult = async () => {
        setSubmitted(true);
        if (result.taskCode && result.description && result.divisionCode) {
            let _result = { ...result };
            if (isEdit) {
                setWaiting(true);
                setSubmitted(false);
                setWaiting(true);
                let data = await putData(`/TaskCategory/Update`, _result, "TaskCategories-Index");
                if (data) {
                    setResults((prev) =>
                        prev.map((item) => {
                            if (item.taskCode === data?.taskCode) {
                                return data;
                            }
                            return item;
                        })
                ...
                setSubmitted(false);
                setWaiting(true);
                let data = await postData(`/TaskCategory/Create`, _result, "TaskCategories-Index");
                if (data) {
                    setResults((prev) => [...prev, data]);
                    setResultDialog(false);
                    setResult(emptyResult);
    ...
    const deleteResult = async () => {
        setWaiting(true);
        let data = await deleteData(`/TaskCategory/Delete?taskCode=${result?.taskCode}`, "TaskCategories-Index");
        if (data) {
            setResults(results.filter((item) => item.taskCode !== data?.taskCode));
            setDeleteResultDialog(false);
            setResult(emptyResult);
        }
        setWaiting(false);
    ...
    const onTaskCodeChange = (e, name) => {
        ...
        if (isString(val)) {
            ...
            setIsString("Only Alphabets allowed");
        }
    };
    const onAutoComplteChange = (e, name) => {
        const val = (e.target && e.target.value) || "";
        let _result = { ...result };
        let ctn = val.divisionCode;
        _result[`${name}`] = ctn;
        setResult(_result);
    };
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h4 className="m-0">Task Categories</h4>
                ...
                <div className="col-12">
                    <div className="card">
                        ...
                        >
                            ...
                            <Column field="taskCode" header="Task" filter sortable className="p-column-title"></Column>
                        ...
                        <Dialog visible={resultDialog} style={{ width: "900px" }} header="" modal className="p-fluid" footer={resultDialogFooter} onHide={hideDialog}>
                            <div className="card p-fluid">
                                <h4 className="text-center">Task Category {isEdit ? " Edit" : " Create"}</h4>
                                <br />
                                <div className="formgrid grid">
                                    <div className="field col">
                                        ...
                                        <Dropdown
                                            ...
                                            onChange={(e) => onAutoComplteChange(e, "divisionCode")}
                                            optionLabel="divisionCode"
                                            filter
                                            filterBy="divisionCode"
                                            placeholder={dialogLoading ? "Loading..." : result.divisionCode || "Choose Division"}
                                        />
                                    ...
                                    <div className="field col">
                                        ...
                                        <InputText id="taskCode" value={result.taskCode} disabled={isEdit} onChange={(e) => onTaskCodeChange(e, "taskCode")} required className={classNames({ "p-invalid": submitted && !result.taskCode })} />
                                        <div className="text-danger">{stringData}</div>
                                        {submitted && !result.taskCode && <small className="p-invalid text-danger">Task Code is required.</small>}
                                    </div>
                                </div>
                                <div className="formgrid grid">
                                    <div className="field col">
                                        <label htmlFor="description">Task Description</label>
                                        <InputTextarea id="description" placeholder="Task description not more than 100 words." value={result.description} onChange={(e) => onInputChange(e, "description")} required className={classNames({ "p-invalid": submitted && !result.description })} />
                                        {submitted && !result.description && <small className="p-invalid text-danger">Task Description is required.</small>}
                                    </div>
                                </div>
                            </div>
                        </Dialog>
                        ...
                        <Dialog visible={uploadDialog} style={{ width: "600px" }} header="" modal footer={uploadDialogFooter} onHide={hideUploadDialog}>
                            <div className="card p-fluid">
                                <header>
                                    <h4 className="text-center">Mass Task Category Uploading</h4>
                                    <br />
                                    <h5>
...
export default React.memo(TaskCategory, comparisonFn);
