...
const fetchData = async () => {
    ...
    localStorage.setItem("specificCoachingReport", JSON.stringify(res?.data));
    return res.data;
};
export const ActionReport = () => {
    ...
    useEffect(() => {
        const storedData = localStorage.getItem("specificCoachingReport");
        if (storedData) {
            try {
                const parsedData = JSON.parse(storedData);
                setAssessment(parsedData?.filter((key) => key.actionplanStatus !== ""));
                setLoading(false);
            } catch (ex) {
                console.log("no data");
            }
        ...
        fetchData()
            .then((data) => {
                setAssessment(data?.filter((key) => key.actionplanStatus !== ""));
            })
            .catch((error) => {
                // Handle any errors
            })
    ...
    const statusTemplate = (data) => {
        return <span className={"customer-badge " + data?.actionplanStatus}>{data?.actionplanStatus}</span>;
    };
                ...
                <div className="col-12">
                    <div className="card">
                        ...
                        >
                            ...
                            <Column field="actionplanStatus" header="Status" body={statusTemplate}></Column>
                        </DataTable>
                    </div>
                </div>
            </div>
